...
build-container:
	$(IMAGE_BUILD_CMD) build --network=host --no-cache -f Dockerfile -t $(FULL_OPERATOR_IMAGE) --build-arg OPENSHIFT_PYTHON_WRAPPER_COMMIT=$(OPENSHIFT_PYTHON_WRAPPER_COMMIT) --build-arg OPENSHIFT_PYTHON_UTILITIES_COMMIT=$(OPENSHIFT_PYTHON_UTILITIES_COMMIT) --build-arg TIMEOUT_SAMPLER_COMMIT=$(TIMEOUT_SAMPLER_COMMIT) --build-arg PYHELPER_UTILS_COMMIT=$(PYHELPER_UTILS_COMMIT) .
